...
RETURN (select ID_TIEMPO from REINUS.BI_TIEMPO
where
...
AS
BEGIN
    INSERT INTO BI_RANGO_HORARIO(RANGO_HORARIO_DESCRIPCION)
    VALUES ('00:00-06:00')
    INSERT INTO BI_RANGO_HORARIO(RANGO_HORARIO_DESCRIPCION)
    VALUES ('06:00-12:00')
    INSERT INTO BI_RANGO_HORARIO(RANGO_HORARIO_DESCRIPCION)
...
AS
BEGIN
    INSERT INTO BI_TIPO_ENVIO(TIPO)
...
AS
BEGIN
    INSERT INTO BI_MEDIO_PAGO(MEDIO_PAGO, TIPO_MEDIO_PAGO)
...
AS
BEGIN
INSERT INTO BI_RUBRO (PRODUCTO_RUBRO_DESCRIPCION)
...
AS
BEGIN
INSERT INTO BI_SUBRUBRO(PRODUCTO_SUB_RUBRO)
...
BEGIN
    ...
    JOIN REINUS.Provincia P ON P.ID_PROVINCIA = L.ID_PROVINCIA
	 ...
	 FROM REINUS.Almacen AS a
	 JOIN REINUS.Domicilio D on a.DOMICILIO_ID = D.ID_DOMICILIO
	 JOIN REINUS.Localidad L  on L.ID_LOCALIDAD = D.LOCALIDAD_ID
	 JOIN REINUS.Provincia P ON P.ID_PROVINCIA = L.ID_PROVINCIA
	 LEFT JOIN REINUS.BI_UBICACION  AS bi_u ON 
        bi_u.UBICACION_PROVINCIA =  P.NOMBRE AND
        bi_u.UBICACION_LOCALIDAD = L.NOMBRE
    WHERE bi_u.ID_UBICACION IS NULL; 

END
GO
-- NO se si falta algun insert mas, hay almacen
...
AS
BEGIN
    INSERT INTO REINUS.BI_PUBLICACION (PUBLICACION_TIEMPO, PUBLICACION_MARCA, PUBLICACION_SUBRUBRO, PUBLICACION_RUBRO)
    SELECT 
        T.ID_TIEMPO, 
		bm.ID_MARCA,
        SR.ID_SUBRUBRO,       
 ...
 JOIN REINUS.BI_MARCA bm on bm.DESCRIPCION = M.MARCA
...
AS
BEGIN
    INSERT INTO REINUS.BI_FACTURA (FACTURA_TIEMPO, FACTURA_UBICACION, CONCEPTO,TOTAL_FACTURADO)
    SELECT 
        T.ID_TIEMPO,          
        UB.ID_UBICACION,  
		con.ID_CONCEPTO,
		p.TOTAL -- el total asi nomas'?
	...
	--Si va concepto , entonces iria solo eso
	JOIN REINUS.Detalle_Factura AS f ON p.FACTURA_ID = f.FACTURA_ID
    JOIN REINUS.Concepto AS con ON con.ID_CONCEPTO = f.CONCEPTO_ID
    JOIN REINUS.BI_CONCEPTO AS c ON c.tipo = con.FACTURA_DET_TIPO;
END
    GROUP BY 
